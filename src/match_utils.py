...
def comU1(x, y):
    result = [layer_utils.cosine_distance(x, y), layer_utils.euclidean_distance(x, y), layer_utils.l1_distance(x, y)] # , compute_euclidean_distance(x, y), compute_l1_distance(x, y)
    return tf.stack(result, axis=1)

def comU2(x, y):
    result = [layer_utils.cosine_distance(x, y), layer_utils.euclidean_distance(x, y)] # , compute_euclidean_distance(x, y)
    return tf.stack(result, axis=1)

def mpcnn_match_func(sent1_blockA, sent2_blockA, sent1_blockB, sent2_blockB, poolings, filter_sizes, num_filters):
    match_dim = 0
    fea_h = []  # len(poolings) * num_filters * [batch, 2]
    with tf.variable_scope("cal_dis_with_alg1_blockA"):
        for i in range(len(poolings)):
            regM1 = tf.concat(sent1_blockA[i], 1)
            regM2 = tf.concat(sent2_blockA[i], 1)
            for j in range(num_filters[0]):
                fea_h.append(comU2(regM1[:, :, j], regM2[:, :, j]))
        match_dim += len(poolings) * num_filters[0] * 2

    fea_a = []  # len(poolings) * len(filter_sizes) * len(filter_sizes) * [batch, 3]
    with tf.variable_scope("cal_dis_with_alg2_blockA"):
        for i in range(len(poolings)):
            for j in range(len(filter_sizes)):
                for k in range(len(filter_sizes)):
                    fea_a.append(comU1(sent1_blockA[i][j][:, 0, :], sent2_blockA[i][k][:, 0, :]))
        match_dim += len(poolings) * len(filter_sizes) * len(filter_sizes) * 3

    fea_b = []  # (len(poolings)-1) * (len(filter_sizes)-1) * num_filters * [batch, 3]
    with tf.variable_scope("cal_dis_with_alg2_blockB"):
        for i in range(len(poolings)-1):
            for j in range(len(filter_sizes)-1):
                for k in range(num_filters[1]):
                    fea_b.append(comU1(sent1_blockB[i][j][:, :, k], sent2_blockB[i][j][:, :, k]))
        match_dim += (len(poolings)-1) * (len(filter_sizes)-1) * num_filters[1] * 3
    
    match_representation = tf.concat(axis=1, values=fea_h + fea_a + fea_b) # [batch_size, match_dim]
    return (match_representation, match_dim)



def siameseLSTM_match_func(question_context_outputs, passage_context_outputs, context_lstm_dim):
    # Sentence matching layer
    with tf.name_scope("match_sentences_context"):
        sentence_difference = question_context_outputs - passage_context_outputs
        sentence_product = question_context_outputs * passage_context_outputs
        # Shape: (batch_size, 4 * 2 * rnn_hidden_size)
        match_representation = tf.concat([question_context_outputs, sentence_product,
                                        sentence_difference, passage_context_outputs], 1)
        match_dim = 4 * 2 * context_lstm_dim # tf.shape(match_representation)[-1]
    return (match_representation, match_dim)
