...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        sess = tf.Session()
        sess.run(initializer, feed_dict={valid_graph.embedding: word_vocab.word_vecs})
        ...
        if options.with_f1_metric:
            ...
            acc = SentenceMatchTrainer.evaluation(sess, valid_graph, testDataStream, outpath=args.out_path, label_vocab=label_vocab)
            metric = acc
            print("Accuracy for test set is: %.5f" % acc)
            print("Metric for test set is: %.5f" % metric)

    # python SentenceMatchDecoder.py --in_path ../data_quora/test.tsv --word_vec_path ../data_quora/wordvec.txt --out_path ../data_quora/logs_bimpm/test.json --model_prefix ../data_quora/logs_bimpm/SentenceMatch.quora